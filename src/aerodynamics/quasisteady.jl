...
function quasisteady1_loads_b(a, b, ρ, a0, α0, u, v, ω)
    ...
    A_b = -2*a0*ρ*b*(v + d*ω - u*α0)*d_b - a0*ρ*(v + d*ω - u*α0)^2
    M_b = -tmp2_b*u*ω + (1/2 + a)*N + (b/2 + a*b)*N_b
...
function quasisteady1_loads_ρ(a, b, ρ, a0, α0, u, v, ω)
    ...
    A_ρ = -a0*b*(v + d*ω - u*α0)^2
    M_ρ = -tmp2_ρ*u*ω + (b/2 + a*b)*N_ρ
...
function quasisteady2_loads(a, b, ρ, a0, α0, u, v, ω, vdot, ωdot)
    ...
    A = -a0*ρ*b*(v + d*ω - u*α0)^2
    # moment at reference point
    M = -tmp2*(vdot/2 + u*ω + (b/8 - a*b/2)*ωdot) + (b/2 + a*b)*N
