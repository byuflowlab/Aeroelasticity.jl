...
``\\begin{bmatrix} \\Delta L & p_b & b_x & b_y & b_z \\end{bmatrix}^T`` for each lifting line
element, followed by the freestream velocity components ``\\begin{bmatrix} V_x &
V_y & V_z \\end{bmatrix}^T`` and density ``\\rho``, followed by the rigid body
properties ``\begin{bmatrix}^T m & I_{xx} & I_{yy} & I_{zz} & I_{xz} & I_{xy} &
I_{yz} \\end{bmatrix}^T``
...
V_x & V_y & V_z \\end{bmatrix}^T`` and density ``\\rho`` as additional parameters.
...
number_of_parameters(::Type{<:LiftingLine}, ::Type{<:GEBT}, ::Type{<:RigidBody}) = 4
