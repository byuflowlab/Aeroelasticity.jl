...
function coupling_rate_jacobian_type(::Type{LiftingLine{NA,TA}}, ::Type{<:RigidBody},
    ...
    if all(isempty.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Empty()
    elseif all(iszero.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Zeros()
    elseif all(isidentity.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Identity()
    elseif all(isinvariant.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Invariant()
    elseif all(isconstant.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Constant()
    elseif all(islinear.(coupling_rate_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Linear()
    else
        return Nonlinear()
...
function coupling_state_jacobian_type(::Type{LiftingLine{NA,TA}}, ::Type{<:RigidBody},
    ...
    elseif all(isidentity.(coupling_state_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Identity()
    elseif all(isinvariant.(coupling_state_jacobian_type.(aero_model_types, Ref(LiftingLineSection),
        flap_model_types, Ref(LiftingLineSectionControl))))
        return Invariant()
...
coupling_parameter_jacobian_type(::Type{<:LiftingLine}, ::Type{<:RigidBody},
...
function get_coupling_inputs!(y, aero::LiftingLine{NA,TA}, dyn::RigidBody,
    ...
    for i = 1:NA
        ...
        dusi = vcat(dvi, dωi) # rigid body rate variables
        dufi = SVector{Nufi}(dufs[i]) # control surface rate variables
        duci = zeros(SMatrix{1,NG}) # controller rate variables
