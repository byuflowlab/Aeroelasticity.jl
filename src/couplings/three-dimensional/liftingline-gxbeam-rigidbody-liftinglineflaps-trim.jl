...
function mass_matrix_type(::Type{LiftingLine{NA,TA}}, ::Type{<:GEBT}, ::Type{<:RigidBody},
...
function state_jacobian_type(::Type{LiftingLine{NA,TA}}, ::Type{<:GEBT}, ::Type{<:RigidBody},
    ::Type{LiftingLineFlaps{NF,NG,TF,TG}}, ::Type{<:Trim}) where {NA,NF,NG,TA,TF,TG}
...
function number_of_parameters(aero::LiftingLine, stru::GEBT, dyn::RigidBody,
    ...
    return 1 + 6*length(stru.icol_pt) + 6*length(stru.icol_beam) + NG - NC
end
...
function get_inputs!(y, aero::LiftingLine{NA,TA}, stru::GEBT, dyn::RigidBody,
    ...
    npadd = number_of_parameters(aero, stru, dyn, flap, ctrl) # number of additional parameters
...
function get_input_mass_matrix!(My, aero::LiftingLine{NA,TA}, stru::GEBT,
    ...
    u, p, t) where {NA,NF,NG,NC,TA,TF,TG}
    ...
    for i = 1:NA
        ...
        loads_duai = vcat(ΔL*f_duai, GXBeam.tilde(pe)*ΔL*f_duai + ΔL*m_duai)
        loads_dPi = vcat(ΔL*f_dPi, GXBeam.tilde(pe)*ΔL*f_dPi + ΔL*m_dPi)
        loads_dHi = vcat(ΔL*f_dHi, GXBeam.tilde(pe)*ΔL*f_dHi + ΔL*m_dHi)
        loads_dufi = vcat(ΔL*f_dufi, GXBeam.tilde(pe)*ΔL*f_dufi + ΔL*m_dufi)
        for ic = 1:NC
    ...
    loads_dV = vcat(Ftot_dV, Mtot_dV)
    loads_dΩ = vcat(Ftot_dΩ, Mtot_dΩ)
...
function get_inputs_from_state_rates(aero::LiftingLine{NA,TA}, stru::GEBT,
    dyn::RigidBody, flap::LiftingLineFlaps{NF,NG,TF,TG}, ctrl::Trim{NC},
    du, u, p, t) where {NA,NF,NG,NC,TA,TF,TG}

    # initialize input vector
    models = (aero, stru, dyn, flap, ctrl)
