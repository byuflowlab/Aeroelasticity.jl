...
function wagner_model(; C1=0.165, C2=0.335, eps1 = 0.0455, eps2 = 0.3)
    ...
    ratejac = Identity()
    statejac = Linear((dx, x, y, p, t) -> wagner_state_jacobian(dx, x, y, p, t; eps1, eps2))
    inputjac = Nonlinear((dx, x, y, p, t) -> wagner_input_jacobian(dx, x, y, p, t; C1, C2, eps1, eps2))
    paramjac = Nonlinear((dx, x, y, p, t) -> wagner_parameter_jacobian(dx, x, y, p, t; C1, C2, eps1, eps2))
    tgrad = Zeros()

    # convenience functions for setting states, inputs, and parameters
    setstate = wagner_setstate!
    setinput = wagner_setinput!
    setparam = wagner_setparam!

    # convenience functions for separating states, inputs, and parameters
    sepstate = wagner_sepstate
    sepinput = wagner_sepinput
    sepparam = wagner_sepparam
...
wagner_setstate!(x; lambda) = x .= lambda

# convenience function for defining this model's input vector
function wagner_setinput!(y; u, v, omega)
    y[1] = u
    y[2] = v
    y[3] = omega
    return y
...
function wagner_setparam!(p; a, b, a0, alpha0, cd0, cm0)
    p[1] = a
    p[2] = b
    p[3] = a0
    p[4] = alpha0
...
wagner_sepstate(x) = (lambda = x,)

# convenience function for separating this model's input vector
wagner_sepinput(y) = (u=y[1], v=y[2], omega=y[3])

# convenience function for separating this model's parameter vector
wagner_sepparam(p) = (a=p[1], b=p[2], a0=p[3], alpha0=p[4], cd0=p[5], cm0=p[6])
